...
The recommended declaration interface is [Standard Schema](https://standardschema.dev/schema), keeping the validator library optional for core while requiring schemas for this SDK's public wire operations. An alternative is JSON Schema as the primary authoring format, with a validator/compiler adapter. Neither is selected yet. [Standard JSON Schema](https://standardschema.dev/json-schema) is a separate export interface, not a guarantee supplied by every Standard Schema validator.
