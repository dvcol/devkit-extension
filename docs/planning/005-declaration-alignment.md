...
A follow-up inspected schema declarations and selected implementation paths in the SHA-512-verified `devframe@1.0.0` and `@devframes/json-render@1.0.0` release artifacts. This is source evidence, with no new runtime experiment. The corresponding source links below identify the audited snapshot.

| Boundary | Observed behavior | Implication for the portable contract |
| --- | --- | --- |
| RPC schema declarations | Positional arguments and the return value accept `StandardSchemaV1`; public function maps infer both from `InferInput` | Standard Schema is an existing validator-neutral integration point. Raw JSON Schema needs a validator adapter. |
| RPC validation | Accepts sync/async validators and checks inputs before the handler and the resolved return afterward. Successful transformed outputs are discarded; extra positional arguments are not rejected by the generic validator | Upstream validation checks original values. We cannot promise coercion, defaults, stripping or exact arity from this path. |
| JSON view validation | A replacement Standard Schema validator must be synchronous. Successful transformed output is discarded. Creation and whole-spec updates validate; `patchState()` does not rerun that schema | Full view/state/action validation needs explicit integration work. A schema option alone does not prove the acceptance criteria. |
| Default JSON component checks | Validates known base-component props with Zod and skips unknown component types | This is not complete structural validation or proof that every custom component is valid. |
| Public metadata conversion | MCP separately asks for `~standard.jsonSchema.input()` targeting draft 2020-12. A missing or throwing converter falls back to a permissive object schema; the same input conversion is used for returns | Standard Schema validation alone cannot guarantee faithful serializable schema metadata. Export support must be specified separately. |

Sources: [RPC definitions and inference](https://github.com/devframes/devframe/blob/18fa60effeb928eb445add2a536cd44906a59981/packages/devframe/src/rpc/types.ts#L94), [function inference helpers](https://github.com/devframes/devframe/blob/18fa60effeb928eb445add2a536cd44906a59981/packages/devframe/src/rpc/utils.ts), [RPC validation](https://github.com/devframes/devframe/blob/18fa60effeb928eb445add2a536cd44906a59981/packages/devframe/src/rpc/validate-io.ts), [JSON view validation](https://github.com/devframes/devframe/blob/18fa60effeb928eb445add2a536cd44906a59981/packages/json-render/src/node/create-view.ts#L119), [MCP schema conversion](https://github.com/devframes/devframe/blob/18fa60effeb928eb445add2a536cd44906a59981/packages/devframe/src/adapters/mcp/to-json-schema.ts).

The owner selected [Standard Schema](https://standardschema.dev/schema) in interview round 3, keeping the validator library optional for core while requiring schemas for this SDK's public wire operations. JSON Schema as the primary authoring format was the alternative. [Standard JSON Schema](https://standardschema.dev/json-schema) is a separate export interface, not a guarantee supplied by every Standard Schema validator. Round 4 selected guard-only semantics and export requirements limited to integrations needing schema inspection, as recorded in the [decision ledger](./005-decisions.md).

Validation checks original values; authors normalize explicitly in their implementation or a handler wrapper. Inferred handler and return types must match those original values. An integration requiring schema export must expose missing or unsupported conversion; upstream's permissive fallback is not evidence of an accurate contract. Metadata does not replace the actual validator.

Validation, serialization and authorization remain distinct. RPC chooses serialization separately through `jsonSerializable`; JSON views use `JSON.stringify()`, which does not prove a lossless round trip. Neither check establishes permission to invoke an operation or access a target. [RPC serialization](https://github.com/devframes/devframe/blob/18fa60effeb928eb445add2a536cd44906a59981/packages/devframe/src/rpc/serialization.ts), [view serialization check](https://github.com/devframes/devframe/blob/18fa60effeb928eb445add2a536cd44906a59981/packages/json-render/src/node/create-view.ts#L80).
