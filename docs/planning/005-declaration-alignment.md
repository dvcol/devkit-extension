...
The owner selected [Standard Schema](https://standardschema.dev/schema) in interview round 3, keeping the validator library optional for core while requiring schemas for this SDK's public wire operations. JSON Schema as the primary authoring format was the alternative. [Standard JSON Schema](https://standardschema.dev/json-schema) is a separate export interface, not a guarantee supplied by every Standard Schema validator. Transformation semantics and export requirements remain open in the [decision ledger](./005-decisions.md).

After selecting the authoring interface, the contract must settle normalization semantics and public schema export. Reusing upstream checks favors validation of original values. Using transformed schema output would require deliberate input/output typing and adapter changes. Missing faithful metadata export must remain visible; upstream's permissive fallback is not evidence of an accurate contract.
