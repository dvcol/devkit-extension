...
A subsequent [executed Vite 8.3 lifecycle trace](../probes/vite-host-lifecycle/README.md) shows that restart completes the replacement server's `configureServer` callbacks before old teardown begins. It awaits both old `closeBundle` and global `closeServer`, but environment shutdown swallows `closeBundle` rejection while `closeServer` rejection remains observable. Native DevTools setup runs inside `configureServer`; an eager portable installation there can therefore overlap its predecessor. Hook names alone are insufficient proof of safe replacement. The local adapter remains valid, but Vite wiring must separately establish activation ordering, readiness and failed-cleanup behavior. Staged setup, middleware mode and config reload still need a real native-host proof before an integration strategy is selected.
