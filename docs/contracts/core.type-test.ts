...
const serverContext = defineNativeContext<{ readonly serverName: string }>({ id: 'example.server-context' });

const titleCapability = defineCapability({
  id: 'example.title',
  version: 1,
  operations: {
    read: defineOperation({ input: z.object({ prefix: z.string() }), output: z.string(), target: 'required' }),
    status: defineOperation({ input: z.object({}), output: z.boolean(), target: 'none' }),
    normalize: defineOperation({ input: z.string().transform(Number), output: z.string().transform(Number), target: 'none' }),
  },
});
...
export const plugin = definePlugin({
  id: 'example.plugin', services: [titleService], actions: [readContribution], extensions: [notification],
});

export async function checkClients(capabilities: CapabilityClient, actions: ActionClient, target: TargetReference): Promise<void> {
...
export function checkContexts(context: BindingContext, native: NativeContextAccess, error: unknown): void {
  ...
  if (isOperationError(error)) error.diagnostic.phase satisfies 'admission' | 'setup' | 'call' | 'cleanup' | 'transport';
}
...
definePlugin({ id: 'example.missing-contract', services: [{ id: 'bad', kind: 'service', execution: serverExecution }] });
