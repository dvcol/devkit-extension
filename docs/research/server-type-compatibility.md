...
A maintained server adapter can retain full declaration checking. A clean, frozen-lock installation of the current public packages passes TypeScript 7.0.2 after three narrowly scoped declaration patches and two dependency metadata corrections. No `skipLibCheck`, ambient `any`, native-context cast or replacement RPC implementation was used. This resolves the bounded declaration feasibility question; issue 6's adapter lifecycle and backend-reset policy remain separate decisions.
...
| Missing `cac`                              | Devframe's shared context declaration imports `CAC` even when no CLI adapter is selected. Its published manifest marks cac as an optional peer.                                                                                                                                            | Satisfy cac 7.0.0 through a package extension attached to Devframe. A direct application dependency also fixed the observed diagnostic.                 | Devframe dependency metadata or separation of CLI declarations |
...
| Missing `whenexpr` through kit             | `devframe/dist/utils/when.d.mts` imports `WhenExpression` from whenexpr, while the published package puts whenexpr only in development dependencies.                                                                                                                                       | Add whenexpr 0.1.2 through the same Devframe package extension.                                                                                         | Devframe declaration bundling or dependency metadata           |
...
The second clean installation used the preserved candidate lock and package-manager patch mechanism, rather than hand-edited dependency files. `check.mjs` runs these five checks and records each exit code:
...
Every configuration has `strict: true` and `skipLibCheck: false`. The scoped fixture verifies numeric values inferred from a namespaced registry on both node and browser contexts. Two expected compiler errors reject assigning those values to strings. This checks the proposed constraint fix without removing the upstream API's existing generic fallback overload. [Final compiler results](../probes/server-type-compatibility/results.json), [scoped fixture](../probes/server-type-compatibility/scoped-typecheck.ts).

## Adoption boundary

The proposal is feasible for the exact tested package graph. No patches were applied to the implementation workspace's dependencies or to neighboring Devframe/DevTools source checkouts, and no upstream PR was opened. Source inspection still found the published scoped constraint form in the local Devframe checkout; the report relies on published artifacts and the clean installed reproduction for its conclusions.

The next maintained server package should use the narrow public context types, preserve these full declaration gates and either adopt reviewed exact-version patches or consume verified upstream fixes. Dependency extensions belong with the affected upstream package so a passing application does not accidentally depend on an unrelated root dependency being hoisted. Published downstream adapters would still need to define how their consumers receive the fixes; a repository-level pnpm patch is not automatically propagated to external consumers.
