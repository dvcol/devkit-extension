...
assert.ok(['shared', 'isolated', 'mixed'].includes(mode));
const configuredPackage = process.env.PROBE_DEVFRAME_PACKAGE;
assert.ok(
  configuredPackage !== undefined && configuredPackage.length > 0,
  'Set PROBE_DEVFRAME_PACKAGE to the copied proposed package',
);
const patchedPackage: string = configuredPackage;
const credentials = {
...
function verifyBrowserResult(browser: Record<string, unknown>): void {
  ...
  assert.deepEqual(
    ...
    mode === 'shared' ? { endpointA: 7, endpointB: 15 } : { endpointA: 5, endpointB: 17 },
  );
  assert.ok(
    endpoints
...
async function buildPage(): Promise<void> {
  await build({
    ...
    resolve: {
      alias: [
        { find: 'devframe/client', replacement: join(patchedPackage, 'dist/client/index.mjs') },
      ],
    },
  });
}
...
async function main(): Promise<void> {
  ...
  console.info(
    styleText('cyan', '🧪 [connection-probe]'),
    'Open built preview:',
    `${origin}/?mode=${mode}`,
  );
