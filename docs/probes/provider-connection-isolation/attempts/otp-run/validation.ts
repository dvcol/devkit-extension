...
export async function readConnectionMeta(base: string): Promise<ConnectionMeta> {
  const value: unknown = await fetch(`${base}__connection.json`).then(response => response.json());
  if (!isRecord(value) || value.backend !== 'websocket') throw new TypeError('Unexpected probe backend');
  let websocket: ConnectionMeta['websocket'];
  if (typeof value.websocket === 'string' || typeof value.websocket === 'number') websocket = value.websocket;
  else if (isRecord(value.websocket) && typeof value.websocket.path === 'string') websocket = { path: value.websocket.path };
  ...
  if (value.jsonSerializableMethods !== undefined) {
    if (!isStringArray(value.jsonSerializableMethods)) throw new TypeError('Invalid serialization method list');
