...
export async function readConnectionMeta(base: string): Promise<ConnectionMeta> {
  ...
  if (
    ...
    !isRecord(value.websocket) ||
    typeof value.websocket.path !== 'string' ||
    !isStringArray(value.jsonSerializableMethods)
  ) {
    throw new TypeError('Unexpected probe connection metadata');
  }
  return {
    backend: 'websocket',
    websocket: { path: value.websocket.path },
    jsonSerializableMethods: value.jsonSerializableMethods,
  };
}
