...
import { createServer, version } from '/Users/dinh-van.colomban/Workspace/private/devkit-extension/node_modules/vite/dist/node/index.js';
...
export default async () => ({plugins: [{name:'candidate-trace',configureServer(server){const generation=globalThis.__devkitProbe.servers.length+1;globalThis.__devkitProbe.servers.push(server);globalThis.__devkitProbe.events.push('configure:'+generation);},closeServer({reason}){globalThis.__devkitProbe.events.push('close:'+reason);}},...await counterHostPlugins('${host}')]});\n`);
  ...
  try {
    ...
    let rejection;
    try {await server.restart();} catch(error){rejection=error.message;}
    assert.equal(rejection,'Provider cleanup is blocked');
    ...
    results.cases.push({host,rejection,beforeManualClose,afterManualClose});
  } finally {
