...
This is currently a local integration package. It does not publish provider discovery, bind portable calls to a remote transport, implement a client router, render a UI, or install Vite reload/close hooks. The genuine kit-context test is not a complete DevTools shell or browser integration test. Host-managed Vite/upstream reload wiring and runnable host examples remain separate work.

The package is private and relies on the repository's exact-version declaration patches and dependency metadata for the released Devframe/hub/kit graph. Those patches preserve upstream runtime JavaScript. This is not a claim that an external consumer can install an unpatched public distribution or that the separate connection-isolation proposal has been adopted.
